...
void append_student_p(string familia, string name, string clas, string happy_birth){
    Student s; string temp, num; int num_temp = 0; 
    s.familia = familia; 
    s.name = name; 
    temp = clas.substr(0, clas.length()-1); s.ch = clas[clas.length() - 1];
    for (int i = 0; i < temp.size(); ++i) { num_temp = 10 * num_temp + (temp[i] - 48);}
    s.clnum = num_temp;
...
int main(){
    ...
    cin >> n;
    for (int i = 0; i < n; ++i){ 
        cin >> familia; cin >> name ; cin >> clas ; cin >> happy_birth;
