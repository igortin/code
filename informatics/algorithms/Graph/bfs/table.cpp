...
bool not_visited(int x, int y, int n, int m) { return x >= 0 && x < n && y >= 0 && y < m && !color[x][y];
}
...
int main() {
	...
	cin >> n >> m;
	for (int i = 0; i < n; i++) {
     for (int j = 0; j < m; j++) { dst[i][j] = 1e9;}
	}
	for (int i = 0; i < n; i++) {
    for (int j = 0; j < m; j++) {
			 	cin >> a;
				if (a != 0) { // cout << "x: " << i << ' '; cout << "y: "<< j << '\n';
					tobfs(i,j,m,n); color_clear(n,m); // print_dst(n, m);
					}
