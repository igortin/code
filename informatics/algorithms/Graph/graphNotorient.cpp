...
void todfs(int i, int count){
	...
	for (size_t j = 0 ; j < count; ++j) {
			if (matrix_smejnost[i][j] == 1){ matrix_smejnost[i][j] = 0; matrix_smejnost[j][i] = 0;

						if (parent[i] == 1 && parent[j] == 0) { parent[j] = 2; todfs(j, count);}
						else if (parent[i] == 2 && parent[j] == 0)  { parent[j] = 1; todfs(j, count);
						}
						else if (parent[j] != 0) {
							int temp_p;
...
int main() {
	...
	for (size_t i=0; i < n; ++i){
		...
		for (size_t i = 0; i < m; ++i){
				 	cin >> a >> b;
					--a; --b;
					matrix_smejnost[a][b] = 1;
					matrix_smejnost[b][a] = 1;
				}
