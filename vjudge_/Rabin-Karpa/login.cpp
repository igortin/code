...
// check account
string check_account(vector<long long>& p_pow, vector<long long>& account_names, string account, int k, string w){
  ...
  return check_account(p_pow, account_names, account, k, w);
}
...
int main() {
  ...
  for (int i = 0; i < count; i++) {
      std::cin >> account;
      string w = account;
      string temp = check_account(p_pow, account_names, account, 0, w);
