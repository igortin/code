    ...
    struct mqueue{
        ...
        string push(int x){
            ...
            if (tail != NULL){                              // node.next set pointer 0x777c20
                tail -> next = item;                        // cout << "tail -> next: " << tail -> next << "\n";
            }
        ...
        int pop(){
            if (head != NULL) {
                int x = head -> val;                 // cout << "pointer to head: " << head << '\n';
                head = head -> next;                 // cout << "change head to pointer: " << head << '\n';
                cnt --;
                cout << x;
                }
            else { cout << "error";}
